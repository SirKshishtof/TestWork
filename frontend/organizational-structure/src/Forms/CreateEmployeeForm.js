...
import { bigButtonStyle } from "../Styles/styles.js";
...
export default function CreateEmployeeForm() {
    ...
    function handleClick_AddEmployee() {
      let leaderCode;
      console.log ()
        if (allOptions.length == 0) {leaderCode = -1
        } else {
          if(selectedOption == "") alert("Выберете руководителя");
          else
          {
            leaderCode = parseInt(selectedOption.slice(0, selectedOption.indexOf(" ", 0)))
            let employee = {
        ...
        <div className="container">
            ...
            >
                <TextField
                    ...
                    margin="normal"
                    onChange={(event) =>
                        setLastName_TextField(event.target.value)
                    }
                />
                <TextField
                    label="Имя"
                    margin="normal"
                    onChange={(event) =>
                        setFirstName_TextField(event.target.value)
                    }
                />
                <TextField
                    label="Отчество"
                    margin="normal"
                    onChange={(event) =>
                        setMiddleName_TextField(event.target.value)
                    }
                />
                <TextField
                    label="Должность"
